...
const pageProperties = [
  { page: "front-page", left: 1696, top: 0, width: 1629, height: 2475 },
  { page: "back-page", left: 0, top: 0, width: 1629, height: 2475 },
  { page: "spine-page", left: 1629, top: 0, width: 67, height: 2475 }
];
...
// path for storing cover pages & pdf
const coverDir = "./output/cover";
const pdfDir = "./output/pdf";
...
function webShot(input) {
  ...
  return new Promise((resolve, reject) => {
    webshot(input, "./output/webshot.png", options, function(err) {
      if (err) reject(err);
...
function cropPage(input, croppingProperties) {
  croppingProperties.map(section => {
    sharp(input)
      .extract({
        left: section.left,
        top: section.top,
      ...
      .toFile(`./output/${section.page}.png`)
      .then(() => {
        console.log(`Generating ${section.page}....`);
      })
      .catch(error => {
...
async function generatePDF(config) {
  try {
    if (config.type === "cover") {
      ...
      // await cropPage("./webshot.png", pageProperties);
